...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- поочередная анимация слогана, вычисление задержек ---
  ...
  if (title) {
    ...
    words.forEach((w, i) => {
      ...
      span.className = 'word';
      span.textContent = w;
      span.style.setProperty('--d', (i * 260) + 'ms'); // скорость слов
  ...
  document.querySelectorAll('a[href]').forEach(a=>{
    ...
    a.addEventListener('click', e=>{
      // якоря на этой же странице не анимируем
  ...
  // --- лёгкий параллакс для карточек/изображений ---
  const parallaxEls = document.querySelectorAll('.parallax');
  const upd = () => {
    const vh = window.innerHeight;
    parallaxEls.forEach(el=>{
      const r = el.getBoundingClientRect();
      const center = (r.top + r.bottom)/2 - vh/2; // позиция относительно центра экрана
      const damp = 0.04; // сила эффекта
      const shift = Math.max(-12, Math.min(12, -center * damp)); // ограничим смещение
...
// --- заглушка изображений (глобальная) ---
